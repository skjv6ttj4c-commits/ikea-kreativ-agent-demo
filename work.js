...
let room3d=null,sceneModule=null,workActive=false,running=false,step=-1,timer=null,due=0,remaining=0,runConfig=null,steeringEvents=[],steeringSerial=0,designVersions=[],compareRooms=null,compareSyncing=false,activeVersion=1,feedbackSelection=null,localFeedbackTags=new Set(),dissatisfactionActive=false,briefEditSession=null;
...
function updateWork(){
 ...
 updatePlan();const process=$('#process-group');if(done){process.open=false;$('#process-status').textContent='已完成';[...$('#plan').children].forEach(node=>node.open=false)}else{process.open=true;$('#process-status').textContent=running?'进行中':'已暂停'}$('#live-feedback').hidden=!done;$('#browse').hidden=!running||step<4||done;
...
const negativeFeedback=/不(?:太|怎么|是很|是特别)?满意|不符合(预期|想法)|不太?行|不太?对|不好|不合适|不合理|不是我想要|不喜欢(这个|这版|目前|现在)?(方案|结果|布局)?|很失望/;
const originalSubmit=$('#input-form').onsubmit;$('#input-form').onsubmit=e=>{if(!workActive)return originalSubmit(e);e.preventDefault();const text=prompt.value.trim();if(!text)return;const supported=/单人床|双人床|更多收纳|增加收纳|高柜|书桌|办公区|两面靠墙|两侧靠墙|预算|动线.{0,6}(不要|不放).{0,4}家具/.test(text),negative=negativeFeedback.test(text),concrete=/太挤|拥挤|位置|摆放|留空|不要|太大|太小|挡住|不方便|风格|颜色|色调|材质|预算|动线|收纳不足|光线|氛围|温馨|简约|家具.{0,4}不喜欢|不喜欢.{0,4}家具/.test(text);const note=$('#steering-message');if(/暂停|停一下/.test(text)){pause();note.className='';note.hidden=false;note.textContent='已暂停。当前草案已保留。';prompt.value='';sync()}else if(negative&&!concrete||dissatisfactionActive&&!concrete)enterDissatisfaction(text);else if(dissatisfactionActive||negative&&concrete){$('#spatial-feedback-text').value='';applySpatialFeedback(feedbackTarget(text),text)}else if(supported){dissatisfactionActive=false;setSpatialFeedbackMode(false);applySteering(text)}else{dissatisfactionActive=false;setSpatialFeedbackMode(false);applySteering(text)}};
