...
let room3d=null,sceneModule=null,workActive=false,running=false,step=-1,timer=null,due=0,remaining=0,runConfig=null,steeringEvents=[],steeringSerial=0,designVersions=[],compareRooms=null,compareSyncing=false,activeVersion=1,feedbackSelection=null,localFeedbackTags=new Set();
...
function enterDissatisfaction(text){if(running)pause();else{clearTimeout(timer);running=false;updateWork()}closeCompare();const note=$('#steering-message');note.className='dissatisfaction';note.replaceChildren();const head=document.createElement('div');head.className='dissatisfaction-head';const said=document.createElement('small');said.textContent='你刚刚说';const quote=document.createElement('blockquote');quote.textContent='“'+text+'”';head.append(said,quote);const body=document.createElement('div');body.className='dissatisfaction-body';const title=document.createElement('h3');title.textContent='先不急着重做，我们一起找出问题';const desc=document.createElement('p');desc.textContent='当前方案已经保留。你可以在右侧点选具体家具或区域，我会只调整受影响的部分。';const actions=document.createElement('div');actions.className='dissatisfaction-actions';const locate=document.createElement('button');locate.className='locate-feedback';locate.textContent='去空间里指出位置 →';locate.onclick=()=>{setSpatialFeedbackMode(true);$('#spatial-feedback-prompt').hidden=false;toast('点击右侧家具或区域即可添加反馈')};const restart=document.createElement('button');restart.className='restart-feedback';restart.textContent='整体方向需要重来';restart.onclick=()=>{setSpatialFeedbackMode(false);editBrief()};actions.append(locate,restart);body.append(title,desc,actions);note.append(head,body);note.hidden=false;setSpatialFeedbackMode(true);$('#scene-status').hidden=false;$('#scene-status-text').textContent='等待你指出需要调整的位置';$('#scene-status').classList.add('paused');prompt.value='';prompt.placeholder='也可以直接说：床的位置不对…';sync();requestAnimationFrame(()=>note.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'center'}))}
function applySpatialFeedback(selection,details){const snapshot={rows:rows.map(r=>({...r})),config:{...runConfig},step,running};clearTimeout(timer);steeringEvents.forEach(e=>{if(e.state==='active')e.state='done'});const leave=/这里应该留空/.test(details),crowded=/太挤/.test(details);if(leave){const remove={bed:'不要床',storage:'不要收纳柜',desk:'不要办公区'}[selection.key];if(remove)parse(remove);else add('lock','此处空间保持留空')}if(crowded)add('prefer',selection.label+'周围需要更宽松');const typed=$('#spatial-feedback-text').value.trim();if(typed)add('prefer',selection.label+'：'+typed);render();summary();runConfig=configFromBrief();room3d.configure(runConfig);step=3;room3d.stage(step);const event={id:++steeringSerial,phase:0,text:selection.label+'：'+details,action:'正在重新安排'+selection.label+'，并保护其他区域',bullets:['重点优化'+selection.label,details,'其他家具与已确认需求保持不变'],state:'active',snapshot};steeringEvents.push(event);running=true;remaining=0;previousPhase=-1;activeVersion=1;$('#version-controls').hidden=true;$('#compare-layer').hidden=true;setSpatialFeedbackMode(false);const note=$('#steering-message');note.className='';note.hidden=true;prompt.value='';prompt.placeholder='随时补充，例如：换成单人床…';sync();updateWork();schedule();requestAnimationFrame(()=>document.querySelector('.steer-event.active')?.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'center'}));toast('已收到，我会只重做这个区域')}
...
const originalSubmit=$('#input-form').onsubmit;$('#input-form').onsubmit=e=>{if(!workActive)return originalSubmit(e);e.preventDefault();const text=prompt.value.trim();if(!text)return;const supported=/单人床|双人床|更多收纳|增加收纳|高柜|书桌|办公区|两面靠墙|两侧靠墙|预算|动线.{0,6}(不要|不放).{0,4}家具/.test(text);const note=$('#steering-message');if(/暂停|停一下/.test(text)){pause();note.className='';note.hidden=false;note.textContent='已暂停。当前草案已保留。';prompt.value='';sync()}else if(negativeFeedback.test(text))enterDissatisfaction(text);else if(supported){setSpatialFeedbackMode(false);applySteering(text)}else{pause();setSpatialFeedbackMode(false);note.className='';note.hidden=false;note.textContent='这项调整可能影响多个条件，请先检查需求。';add('prefer',text);render();summary();const btn=document.createElement('button');btn.textContent='查看并确认需求';btn.onclick=editBrief;note.append(btn);prompt.value='';sync();note.scrollIntoView({behavior:'smooth',block:'nearest'})}};
